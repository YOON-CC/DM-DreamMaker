...
import {
  ...
  addITextToCanvas,
  addButtonToCanvas,
} from "../utils/AddCanvasUtils";
import {
  handleScalingRect,
  handleScalingCircle,
  handleScalingTriangle,
  handleScalingTextbox,
  handleScalingImage,
  handleScalingGroup,
} from "../utils/ScalingCanvasUtils";
import {
  handleChangeFontSizeInput,
  handleFontWeightChange,
  handleFontToItalicChange,
  handleFontShadowChange,
} from "../utils/FontOptionUtils";
import {
  applyShadow_0_0,
  applyShadow_0_1,
  applyShadow_0_2,
...
const Editor = () => {
  ...
  const [backgroundColorToggleBtn, setBackgroundColorToggleBtn] =
    useState(false); // 초기 배경색 설정
  ...
  for (let i = 0; i < divCount; i++) {
    ...
    generatedDivs.push(
      <div key={i} id="editor_body_left_input_container_info_container_frame">
        <div id="editor_body_left_input_container_info_container_frame_title">
          {keyName}
        </div>
        <input
          id="editor_body_left_input_container_info_container_frame_input"
          onChange={(event) => handleKeyInputChange(event, i)}
        ></input>
      </div>
    );
  }
  ...
  const [objectCoordinates_center, setObjectCoordinates_center] = useState({
    center_x: 0,
    center_y: 0,
  });
    ...
    if (canvasRef.current) {
      ...
      canvas.on("mouse:up", (options) => {
        ...
        const Elements1 = document.getElementsByClassName(
          "editor_body_right_style_container_1_info_container"
        ) as HTMLCollectionOf<HTMLElement>;
        const Elements2 = document.getElementsByClassName(
          "editor_body_right_style_container_2_info_container"
        ) as HTMLCollectionOf<HTMLElement>;
        ...
        if (activeObject && activeObject.type === "textbox") {
          ...
          for (let i = 0; i < Elements2.length; i++) {
            if (i === 1 || i === 2) Elements2[i].style.display = "none";
          }
          canvas.renderAll();
      ...
      const handleAddITextShape = () => {
        if (canvas) {
          addITextToCanvas(canvas);
        }
      };
      const handleAddButtonShape = () => {
        if (canvas) {
          addButtonToCanvas(canvas);
        }
      };
      ...
      const handleCopyObject = () => {
        ...
        if (activeObject?.type === "activeSelection") {
          // activeSelection인 객체는 복사 안되도록하는 로직
          return;
        }
      ...
      const handleKeyDown = (event: KeyboardEvent) => {
        ...
        if (activeObject) {
          ...
          const centerX =
            (activeObject.left ?? 0) + (activeObject.width ?? 0) / 2;
          const centerY =
            (activeObject.top ?? 0) + (activeObject.height ?? 0) / 2;
          setObjectCoordinates({
      ...
      const handleObjectToGroup = () => {
        ...
        if (activeObject) {
          const groupNum = activeObject
            .getObjects()
            .filter((obj) => obj?.type === "group").length;
      ...
      const handleObjectToApi = () => {
        try {
          ...
          if (activeObject) {
            const numIText = activeObject
              .getObjects()
              .filter((obj) => obj?.type === "i-text").length;
            const numRect = activeObject
              .getObjects()
              .filter((obj) => obj?.type === "rect").length;
      ...
      const changeFontSizeInputButton = document.getElementById(
        "change-fontsize-input-button"
      );
      changeFontSizeInputButton?.addEventListener(
        "click",
        handleChangeFontSize
      );

      const changeRadiusInputButton = document.getElementById(
        "change-radius-input-button"
      );
      changeRadiusInputButton?.addEventListener(
        "click",
        handleChangeRadiusInput
      );

      const changeBorderWidthInputButton = document.getElementById(
        "change-borderwidth-input-button"
      );
      changeBorderWidthInputButton?.addEventListener(
        "click",
        handleChangeBorderWidthInput
      );
      ...
      const sendBackwardsButton = document.getElementById(
        "send-backwards-button"
      );
      sendBackwardsButton?.addEventListener("click", handleSendBackwards);

      const bringForwardButton = document.getElementById(
        "bring-forward-button"
      );
      bringForwardButton?.addEventListener("click", handleBringForward);

      const addImageShapeButton = document.getElementById(
        "add-image-shape-button"
      );
      addImageShapeButton?.addEventListener("click", handleAddImageShape);

      const addITextShapeButton = document.getElementById(
        "add-IText-shape-button"
      );
      addITextShapeButton?.addEventListener("click", handleAddITextShape);

      const deleteITextShapeButton = document.getElementById(
        "delete-IText-shape-button"
      );
      deleteITextShapeButton?.addEventListener("click", handleDeleteITextShape);

      const addButtonShapeButton = document.getElementById(
        "add-Button-shape-button"
      );
      ...
      const fontWeightButton = document.getElementById(
        "change-fontWeight-button"
      );
      fontWeightButton?.addEventListener("click", handleFontWeight);

      const fontItalicButton = document.getElementById(
        "change-fontItalic-button"
      );
      fontItalicButton?.addEventListener("click", handleFontToItalic);

      const fontShadowButton = document.getElementById(
        "change-fontShadow-button"
      );
      ...
      return () => {
        ...
        addITextShapeButton?.removeEventListener(
          "click",
          handleDeleteITextShape
        );
        addButtonShapeButton?.removeEventListener(
          "click",
          handleAddButtonShape
        );
  ...
  return (
    <div className="body">
      <div className="body_cover">
        ...
        <div className="body_cover_text">
          전체화면 그리고 화면비율을 🔍축소해서 사용해주세요.
        </div>
      </div>
      ...
      <div className="editor_header">
        ...
        <div className="editor_header_tool_container">
          <div
            className="editor_header_tool_container_btn"
          ...
          <div
            id="add-textbox-button"
            onClick={() => !showTool && setShowTool(true)}
          >
            <img
              src="../images/text.png"
              style={{
      ...
      <div className="editor_body">
        ...
        <div className="editor_body_left">
          {showComplete && (
            <div className="apiComplete">
              ...
              <div className="apiCompleteToClear" onClick={handleResetApiLogic}>
                초기화
              </div>
            </div>
          )}
          ...
          <div className="editor_body_left_http_container">
            <div className="editor_body_left_http_container_title">
              HTTP(S) 메서드
            </div>
            <div className="editor_body_left_http_container_info_container">
              <button
                className="editor_body_left_http_container_info_container_title_1"
                onClick={() => setSelectedHttpMethod("GET")}
                style={{
                  backgroundColor:
              ...
              <button
                className="editor_body_left_http_container_info_container_title_2"
                onClick={() => setSelectedHttpMethod("POST")}
                style={{
                  backgroundColor:
          ...
          <div className="editor_body_left_input_container">
            <div className="editor_body_left_input_container_title">
              입력요소 추가
            </div>
            <div className="editor_body_left_input_container_info_container">
              ...
              <div className="editor_body_left_input_container_info_container_btn_container">
                <button id="add-IText-shape-button" onClick={addITextAndDiv}>
                  추가
                </button>
                <button
                  id="delete-IText-shape-button"
                  onClick={addITextAndDivDelete}
                >
                  삭제
                </button>
              </div>
              ...
              <div
                className="editor_body_left_input_container_title"
              ...
              <div className="editor_body_left_apply_container">
                {(selectedHttpMethod === "" ||
                  ...
                  inputValues.length !== divCount ||
                  inputValues.some((value) => value.trim().length === 0)) && (
        ...
        <div className="editor_body_center">
          <div className="editor_body_style_container_info_container">
            <div className="editor_body_style_container_info_container_title">
              background color
            </div>
            <div className="editor_body_style_container_info_container_frame">
              <HexColorInput
                className="editor_body_style_container_info_container_container_info"
                color={backgroundColor}
                onChange={handleBackgroundColorChange}
              />
              <div
                className="editor_select_color_btn"
                onClick={() =>
                  setBackgroundColorToggleBtn(!backgroundColorToggleBtn)
                }
              ></div>
        ...
        <div className="editor_body_right">
          <div className="editor_body_right_size_container">
            ...
            <div className="editor_body_right_size_container_info_container">
              <div className="editor_body_right_size_container_info_container_title_1">
                W
              </div>
              <div className="editor_body_right_size_container_info_container_info_1">
                {objectSize.width !== undefined
                  ? objectSize.width.toFixed(2)
                  : "N/A"}
              </div>
              <div className="editor_body_right_size_container_info_container_title_2">
                H
              </div>
              <div className="editor_body_right_size_container_info_container_info_2">
                {objectSize.height !== undefined
                  ? objectSize.height.toFixed(2)
                  : "N/A"}
              </div>
            </div>
          </div>
          <div className="editor_body_right_location_container">
            <div className="editor_body_right_location_container_title">
              위치
            </div>
            <div className="editor_body_right_location_container_info_container">
              <div className="editor_body_right_location_container_info_container_title_1">
                X
              </div>
              <div className="editor_body_right_location_container_info_container_info_1">
                {objectCoordinates_center.center_x.toFixed(2)}
              </div>
              <div className="editor_body_right_location_container_info_container_title_2">
                Y
              </div>
              <div className="editor_body_right_location_container_info_container_info_2">
                {objectCoordinates_center.center_y.toFixed(2)}
              </div>
            </div>
          </div>
          <div className="editor_body_right_order_container">
            <div className="editor_body_right_order_container_title">
              레이어 순서
            </div>
          ...
          <div className="editor_body_right_style_container">
            <div className="editor_body_right_style_container_title">
              스타일
            </div>
            <div className="editor_body_right_style_container_2_info_container">
              <div className="editor_body_right_style_container_2_info_container_title">
                background color
              </div>
              <div className="editor_body_right_style_container_2_info_container_frame">
                <HexColorInput
                  className="editor_body_right_style_container_2_info_container_info"
                  color={color}
            ...
            <div className="editor_body_right_style_container_1_info_container">
              <div className="editor_body_right_style_container_1_info_container_title">
                border radius
              </div>
            ...
            <div className="editor_body_right_style_container_2_info_container">
              ...
              <div className="editor_body_right_style_container_2_info_container_frame">
                ...
                <div
                  className="editor_select_color_btn"
                  onClick={() => setBorderColorToggle(!borderColorToggle)}
                ></div>
              </div>
            </div>
            ...
            <div className="editor_body_right_style_container_1_info_container">
              ...
              <div className="editor_body_right_style_container_1_info_container_btn">
                <button
                  id="select-shadow-button"
                  onClick={() => setShadowOptionToggle(!shadowOptionToggle)}
                >
                  select
                </button>
              </div>
              ...
              <>
                <div className="shadow_size_controll_container">
                  ...
                  <input
                    ...
                    value={shadowSize}
                    onChange={(e) =>
                      handleShadowSizeChange(parseInt(e.target.value))
                    }
                  />
                  ...
                  <input
                    ...
                    value={shadowOffsetX}
                    onChange={(e) =>
                      handleShadowOffsetXChange(parseInt(e.target.value))
                    }
                  />
                  <div className="shadow_size_controll_container_frame">
                    <label>offsetY:</label>
                  ...
                  <input
                    ...
                    value={shadowOffsetY}
                    onChange={(e) =>
                      handleShadowOffsetYChange(parseInt(e.target.value))
                    }
                  />
                </div>
              </>
            ...
            {shadowToggle && (
              <div className="shadow_Frame">
                <button
                  id="apply-shadow-button_0_0"
                  onClick={handleApplyShadow_0_0}
                ></button>
                <button
                  id="apply-shadow-button_0_1"
                  onClick={handleApplyShadow_0_1}
                ></button>
                <button
                  id="apply-shadow-button_0_2"
                  onClick={handleApplyShadow_0_2}
                ></button>
                <button
                  id="apply-shadow-button_1_0"
                  onClick={handleApplyShadow_1_0}
                ></button>
                <button
                  id="apply-shadow-button_1_1"
                  onClick={handleApplyShadow_1_1}
                ></button>
                <button
                  id="apply-shadow-button_1_2"
                  onClick={handleApplyShadow_1_2}
                ></button>
                <button
                  id="apply-shadow-button_2_0"
                  onClick={handleApplyShadow_2_0}
                ></button>
                <button
                  id="apply-shadow-button_2_1"
                  onClick={handleApplyShadow_2_1}
                ></button>
                <button
                  id="apply-shadow-button_2_2"
                  onClick={handleApplyShadow_2_2}
                ></button>
              </div>
            )}
            ...
            <div className="editor_body_right_style_container_2_info_container">
              ...
              <div className="editor_body_right_style_container_2_info_container_frame">
                ...
                <div
                  className="editor_select_color_btn"
                  onClick={() => setShadowColorToggle(!shadowColorToggle)}
                ></div>
              </div>
            </div>
            {shadowColorToggle && (
