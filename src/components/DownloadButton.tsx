...
const DownloadButton: React.FC<DownloadButtonProps> = ({
    ...
    if (canvas) {
      ...
      canvasObjects.forEach((object) => {
        if (object instanceof fabric.Group) {
          ...
          const newX =
            centerX +
            (groupLeft - centerX) * Math.cos(angleInRadians) -
            (groupTop - centerY) * Math.sin(angleInRadians);
          const newY =
            centerY +
            (groupLeft - centerX) * Math.sin(angleInRadians) +
            (groupTop - centerY) * Math.cos(angleInRadians);
          ...
          if (object.name && object.name.startsWith("api_")) {
            ...
            object.forEachObject((groupObject) => {
              if (groupObject instanceof fabric.IText) {
                ...
                //
                const groupObjectWidth =
                  (((groupObject.width ?? 0) * (groupObject.scaleX ?? 0)) /
                    gw) *
                  100;
                const groupObjectHight =
                  (((groupObject.height ?? 0) * (groupObject.scaleY ?? 0)) /
                    gh) *
                  100;
                ...
                const fontShadow = (groupObject.shadow as unknown as Shadow)
                  .color;
              ...
              if (groupObject instanceof fabric.Rect) {
                ...
                const groupObjectWidth =
                  ((((groupObject.width ?? 0) + groupStrokeWidth) *
                    (groupObject.scaleX ?? 0)) /
                    gw) *
                  100;
                const groupObjectHight =
                  ((((groupObject.height ?? 0) + groupStrokeWidth) *
                    (groupObject.scaleY ?? 0)) /
                    gh) *
                  100;

                const oinnerw =
                  (((groupObject.width ?? 0) - groupStrokeWidth * 1.76) /
                    (groupObject.width ?? 0)) *
                  100; // 경계선 있을시 내부 도형
                const oinnerh =
                  (((groupObject.height ?? 0) - groupStrokeWidth * 1.76) /
                    (groupObject.height ?? 0)) *
                  100; // 경계선 있을시 내부 도형
                ...
                const osx =
                  (groupObject.shadow as unknown as Shadow).offsetX * 2;
                const osy =
                  (groupObject.shadow as unknown as Shadow).offsetY * 2;
            ...
            object.forEachObject((groupObject) => {
              ...
              if (groupObject instanceof fabric.Ellipse) {
                ...
                const osc =
                  (groupObject.shadow as Shadow)?.color ?? "transparent";
        ...
        if (object instanceof fabric.Rect) {
          ...
          const newX =
            centerX +
            (originalLeft - centerX) * Math.cos(angleInRadians) -
            (originalTop - centerY) * Math.sin(angleInRadians);
          const newY =
            centerY +
            (originalLeft - centerX) * Math.sin(angleInRadians) +
            (originalTop - centerY) * Math.cos(angleInRadians);
          ...
          const oinnerw =
            (((object.width ?? 0) - strokeWidth * 1.76) / (object.width ?? 0)) *
            100; // 경계선 있을시 내부 도형
          const oinnerh =
            (((object.height ?? 0) - strokeWidth * 1.76) /
              (object.height ?? 0)) *
            100; // 경계선 있을시 내부 도형
        ...
        if (
          ...
          typeof object.rx !== "undefined" &&
          typeof object.ry !== "undefined"
        ) {
          // Fabric.js에서 원 객체인 경우
          const originalLeft = object.left ?? 0; // 부모 태그로부터의 left
          const originalTop = object.top ?? 0; // 부모 태그로부터의 top
        ...
        if (object instanceof fabric.Image) {
          ...
          const ot = ((newY ?? 0) / 500) * 100;
          const ow =
            (((object.width ?? 0) * (object.scaleX ?? 0)) / 1000) * 100;
          const oh =
            (((object.height ?? 0) * (object.scaleY ?? 0)) / 500) * 100;

          console.log(
            "dd",
            (object.width ?? 0) * (object.scaleX ?? 0),
            (object.height ?? 0) * (object.scaleY ?? 0)
          );
  ...
  return (
    <button className="downloadbutton" onClick={handleDownloadCanvasAsHtml}>
      다운로드
    </button>
  );
};
