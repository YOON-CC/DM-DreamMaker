...
const LoadButton: React.FC<LoadButtonProps> = ({ canvas }) => {
            ...
            if (objInfo.objects && objInfo.objects.length > 0) {
              ...
              objInfo.objects.forEach((groupObjInfo: any) => {
                switch (groupObjInfo.type) {
                  ...
                  case "textbox":
                    groupObjects.push(
                      new fabric.Textbox(groupObjInfo.text, groupObjInfo)
                    );
                    break;
                  case "image":
                    const imgElement = new Image();
                    ...
                    groupObjects.push(
                      new fabric.Image(imgElement, groupObjInfo)
                    ); // 그룹에 이미지를 push
                    break;
                  default:
                    break;
  ...
  return canvas ? (
    <button className="loadbutton" onClick={handleLocalStorageToCanvas}>
      불러오기
    </button>
  ) : null;
};
